...
import com.nmc.speedviewlib.util.OnSectionChangeListener;
...
public abstract class Gauge extends View {
    ...
    private com.nmc.speedviewlib.util.OnLevelChangeListener OnLevelChangeListener;
    ...
    /** good Level area */
    private int goodLevelPercent = 10;
    /** satisfactory Level area */
    private int satisfactoryLevelPercent = 20;
    /** moderate Level area */
    private int moderateLevelPercent = 40;
    /** poor Level area */
    private int poorLevelPercent = 60;
    /** very poor Level area */
    private int veryPoorLevelPercent = 80;
    /** severe Level area
    private int severePercent = 100;*/

    public static final byte GOOD_LEVEL_SECTION = 1;
    public static final byte MODERATE_LEVEL_SECTION = 2;
    public static final byte SATISFACTORY_LEVEL_SECTION = 3;
    public static final byte POOR_LEVEL_SECTION = 4;
    public static final byte VERY_POOR_LEVEL_SECTION = 5;
    public static final byte SEVERE_SECTION = 6;
    private byte section = GOOD_LEVEL_SECTION;
    ...
    private void initAttributeSet(Context context, AttributeSet attrs) {
        ...
        goodLevelPercent = a.getInt(R.styleable.Gauge_sv_goodLevelPercent, goodLevelPercent);
        satisfactoryLevelPercent = a.getInt(R.styleable.Gauge_sv_satisfactoryLevelPercent, satisfactoryLevelPercent);
        moderateLevelPercent = a.getInt(R.styleable.Gauge_sv_moderateLevelPercent, moderateLevelPercent);
        poorLevelPercent = a.getInt(R.styleable.Gauge_sv_poorLevelPercent, poorLevelPercent);
        veryPoorLevelPercent = a.getInt(R.styleable.Gauge_sv_veryPoorLevelPercent, veryPoorLevelPercent);
    ...
    private void checkSpeedometerPercent() {
        if (goodLevelPercent > satisfactoryLevelPercent)
            throw new IllegalArgumentException("goodLevelPercent must be smaller than satisfactoryLevelPercent");
        if (satisfactoryLevelPercent > moderateLevelPercent)
            throw new IllegalArgumentException("satisfactoryLevelPercent must be smaller than moderateLevelPercent");
        if (moderateLevelPercent > poorLevelPercent)
            throw new IllegalArgumentException("moderateLevelPercent must be smaller than poorLevelPercent");
        if (poorLevelPercent > veryPoorLevelPercent)
            throw new IllegalArgumentException("poorLevelPercent must be smaller than veryPoorLevelPercent");
        if (goodLevelPercent > 100 || goodLevelPercent < 0)
            throw new IllegalArgumentException("goodLevelPercent must be between [0, 100]");
        if (satisfactoryLevelPercent > 100 || satisfactoryLevelPercent < 0)
            throw new IllegalArgumentException("satisfactoryPercent must be between [0, 100]");
        if (moderateLevelPercent > 100 || moderateLevelPercent < 0)
            throw new IllegalArgumentException("moderateLevelPercent must be between [0, 100]");
        if (poorLevelPercent > 100 || poorLevelPercent < 0)
            throw new IllegalArgumentException("poorLevelPercent must be between [0, 100]");
        if (veryPoorLevelPercent > 100 || veryPoorLevelPercent < 0)
            throw new IllegalArgumentException("veryPoorLevelPercent must be between [0, 100]");
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (newSpeed != currentIntSpeed && OnLevelChangeListener != null) {
            ...
            while (currentIntSpeed != newSpeed) {
                ...
                OnLevelChangeListener.onSpeedChange(this, isSpeedUp, byTremble);
            }
        }
    ...
    /**
     * draw unit and speed text at {@link #speedTextPosition},
     ...
     * <p>
     *     safe method to handle all speed values in {@link #OnLevelChangeListener}.
     * </p>
     ...
     * @param OnLevelChangeListener maybe null, The callback that will run.
     */
    public void setOnLevelChangeListener(com.nmc.speedviewlib.util.OnLevelChangeListener OnLevelChangeListener) {
    ...
    /**
     * @return the long of good level area (good level section) as percent.
     */
    public int getGoodLevelPercent() {
        return goodLevelPercent;
    }

    /**
     * @return the long of good level area (good level section) as Offset [0, 1].
     */
    public float getGoodLevelOffset() {
        return goodLevelPercent *.01f;
    }

    /**
     * to change good level area (good level section).
     * @param goodLevelPercent the long of good level area as percent,
     *                        must be between {@code [0,100]}.
     * @throws IllegalArgumentException if {@code goodLevelPercent} out of range.
     * @throws IllegalArgumentException if {@code goodLevelPercent > satisfactoryLevelPercent}.
     */
    public void setGoodLevelPercent(int goodLevelPercent) {
        this.goodLevelPercent = goodLevelPercent;
        checkSpeedometerPercent();
        if (!attachedToWindow)
            return;
        updateBackgroundBitmap();
    ...
    /**
     * @return the long of satisfactory level area (satisfactory level section) as percent.
     */
    public int getsatisfactoryLevelPercent() {
        return satisfactoryLevelPercent;
    }

    /**
     * @return the long of satisfactory level area (satisfactory level section) as Offset [0, 1].
     */
    public float getSatisfactoryLevelOffset() {
        return satisfactoryLevelPercent *.01f;
    }

    /**
     * to change satisfactory level area (satisfactory level section).
     * @param satisfactoryLevelPercent the long of satisfactory level area as percent,
     *                        must be between {@code [0,100]}.
     * @throws IllegalArgumentException if {@code satisfactoryLevelPercent} out of range.
     * @throws IllegalArgumentException if {@code satisfactoryLevelPercent > moderateLevelPercent}.
     */
    public void setSatisfactoryLevelPercent(int satisfactoryLevelPercent) {
        this.satisfactoryLevelPercent = satisfactoryLevelPercent;
        checkSpeedometerPercent();
        if (!attachedToWindow)
            return;
        updateBackgroundBitmap();
    ...
    /**
     * @return the long of Moderate level area (Moderate level section) as percent.
     */
    public int getModerateLevelPercent() {
        return moderateLevelPercent;
    }

    /**
     * @return the long of Moderate level area (Moderate level section) as Offset [0, 1].
     */
    public float getModerateLevelOffset() {
        return moderateLevelPercent *.01f;
    }

    /**
     * to change Moderate level area (Moderate level section).
     * @param moderateLevelPercent the long of Moderate level area as percent,
     *                        must be between {@code [0,100]}.
     * @throws IllegalArgumentException if {@code ModerateLevelPercent} out of range.
     * @throws IllegalArgumentException if {@code ModerateLevelPercent < poorLevelPercent}.
     */
    public void setModerateLevelPercent(int moderateLevelPercent) {
        this.moderateLevelPercent = moderateLevelPercent;
        checkSpeedometerPercent();
        if (!attachedToWindow)
            return;
        updateBackgroundBitmap();
    ...
    /**
     * @return the long of Poor level area (Poor level section) as percent.
     */
    public int getPoorLevelPercent() {
        return moderateLevelPercent;
    }

    /**
     * @return the long of Poor level area (Poor level section) as Offset [0, 1].
     */
    public float getPoorLevelOffset() {
        return poorLevelPercent *.01f;
    }

    /**
     * to change Poor level area (Poor level section).
     * @param poorLevelPercent the long of Poor level area as percent,
     *                        must be between {@code [0,100]}.
     * @throws IllegalArgumentException if {@code PoorLevelPercent} out of range.
     * @throws IllegalArgumentException if {@code PoorLevelPercent < VeryPoorLevelPercent}.
     */
    public void setPoorLevelPercent(int poorLevelPercent) {
        this.poorLevelPercent = poorLevelPercent;
        checkSpeedometerPercent();
        if (!attachedToWindow)
            return;
        updateBackgroundBitmap();
    ...
    /**
     * @return the long of VeryPoor level area (VeryPoor level section) as percent.
     */
    public int getVeryPoorLevelPercent() {
        return veryPoorLevelPercent;
    }

    /**
     * @return the long of VeryPoor level area (VeryPoor level section) as Offset [0, 1].
     */
    public float getVeryPoorLevelOffset() {
        return veryPoorLevelPercent *.01f;
    }

    /**
     * to change VeryPoor level area (VeryPoor level section).
     * @param veryPoorLevelPercent the long of VeryPoor level area as percent,
     *                        must be between {@code [0,100]}.
     * @throws IllegalArgumentException if {@code VeryPoorLevelPercent} out of range.
     * @throws IllegalArgumentException if {@code VeryPoorLevelPercent < SevereLevelPercent}.
     */
    public void setVeryPoorLevelPercent(int veryPoorLevelPercent) {
        this.veryPoorLevelPercent = veryPoorLevelPercent;
        checkSpeedometerPercent();
        if (!attachedToWindow)
            return;
        updateBackgroundBitmap();
    ...
    /**
     * check if current level in <b>Good Level Section</b>.
     * @return true if current level is only in Good Level Section.
     *
     * @see #setGoodLevelPercent(int)
     */
    public boolean isInGoodSection() {
        return (maxSpeed - minSpeed)*getGoodLevelOffset() + minSpeed >= currentSpeed;
    }

    /**
     * check if current level in <b>Satisfactory Level Section</b>.
     * @return true if current level is only in Satisfactory Section.
     *
     * @see #setSatisfactoryLevelPercent(int)
     */
    public boolean isInSatisfactorySection() {
        return (maxSpeed - minSpeed)*getSatisfactoryLevelOffset() +
                minSpeed >= currentSpeed && !isInGoodSection();
    }

    /**
     * check if current level in <b>Moderate Level Section</b>.
     * @return true if current level is only in Moderate Section.
     *
     * @see #setModerateLevelPercent(int)
     */
    public boolean isInModerateSection() {
        return (maxSpeed - minSpeed)*getModerateLevelOffset() +
                minSpeed >= currentSpeed && !isInSatisfactorySection();
    }

    /**
     * check if current level in <b>Poor Level Section</b>.
     * @return true if current level is only in Poor Section.
     *
     * @see #setPoorLevelPercent(int)
     */
    public boolean isInPoorSection() {
        return (maxSpeed - minSpeed)*getPoorLevelOffset() +
                minSpeed >= currentSpeed && !isInModerateSection();
    }

    /**
     * check if current level in <b>VeryPoor Level Section</b>.
     * @return true if current level is only in VeryPoor Section.
     *
     * @see #setVeryPoorLevelPercent(int)
     */
    public boolean isInVeryPoorSection() {
        return (maxSpeed - minSpeed)*getVeryPoorLevelOffset() +
                minSpeed >= currentSpeed && !isInPoorSection();
    }

    /**
     * check if current level in <b>Severe Section</b>.
     * @return true if current level is only in Severe Section.
     */
    public boolean isInSevereSection() {
        return currentSpeed > (maxSpeed - minSpeed)*getVeryPoorLevelOffset() + minSpeed;
    }
    ...
    public byte getSection() {
        if (isInGoodSection())
            return GOOD_LEVEL_SECTION;
        else if (isInSatisfactorySection())
            return SATISFACTORY_LEVEL_SECTION;
        else if (isInModerateSection())
            return MODERATE_LEVEL_SECTION;
        else if (isInPoorSection())
            return POOR_LEVEL_SECTION;
        else if (isInVeryPoorSection())
            return VERY_POOR_LEVEL_SECTION;
        else
            return SEVERE_SECTION;
    }
    ...
    public enum Position {
        ...
        , IN_VIEW (.5f , 0f  , .5f , 0f  , 0  , 4)
        , IN_CENTER_OF_VIEW (.5f , 0f  , .5f , 0f  , 0  , 7);
